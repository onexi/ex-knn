...
exercise.guessType = function(k){
    ...
    var nearNeigh = data.slice(0,k);

    for (var i = 0; i<k; i++){
        var nearNeighType = nearNeigh[i].type;
        
        if (nearNeighType === 'apartment') types.apartment +=1;
        if (nearNeighType === 'flat') types.flat +=1;
        if (nearNeighType === 'house') types.house +=1;
    }
    
    var guess = {type: false, count: 0}; //initialize guess variable
